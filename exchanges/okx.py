...
class OKXExchange(BaseExchange):
    """OKX exchange implementation for simulation"""
    ...
    async def create_order(self, side: str, amount: float, price: float, symbol: str = None) -> Dict[str, Any]:
        ...
        return {
            'id': f"okx_order_{random.randint(10000, 99999)}",
            'symbol': symbol,
            'side': side,
            'amount': amount,
            'price': price,
    ...
    async def get_supported_pairs(self) -> list:
        ...
        if self.listing_counter % 10 == 0 and 'SOL/USDT' not in self.base_prices:
            self.base_prices['SOL/USDT'] = 100.0
            self.volatilities['SOL/USDT'] = 0.004
            self.logger.info("NEW LISTING: SOL/USDT now available on OKX")
